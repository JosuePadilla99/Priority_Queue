...
void Priority_Queue::buildHeap(vector<int>& inputVec){
    /*This function will be used to build the initial heap from the user input array*/
    capacity = inputVec.size();
    size = capacity;
    queue = inputVec;
...
void Priority_Queue::heapify(int& child){
    ...
    if((rightChild < size) && (queue[rightChild] < queue[smallestValue])){
...
void Priority_Queue::insert(int& value){
    ...
    size++;
    int i = size - 1; // setting i to the end index of the array
    queue[i] = value; // setting the new node to insert at the end of the array
